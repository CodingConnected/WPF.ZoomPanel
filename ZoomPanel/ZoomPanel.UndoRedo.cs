...
namespace CodingConnected.ZoomPanel
{
    public partial class ZoomPanel
    {
        ...
        /// </summary>
        public ICommand RedoZoomCommand => _redoZoomCommand ??=
             new RelayCommand(RedoZoom, () => CanRedoZoom);

        /// <summary>
        /// Command to implement Undo
        /// </summary>
        public ICommand UndoZoomCommand => _undoZoomCommand ??=
        ...
        private bool CanRedoZoom => _redoStack.Count != 0;

        private bool CanUndoZoom => _undoStack.Count != 0;
        ...
        public void DelayedSaveZoom1500Miliseconds()
        {
            ...
            {
                if (_undoStack.Count != 0 && _viewportZoomCache.Equals(_undoStack.Peek()))
                {
                    return;
                }
                ...
                _redoStack.Clear();
                _undoZoomCommand?.NotifyCanExecuteChanged();
                _redoZoomCommand?.NotifyCanExecuteChanged();
            })).Nudge();
        }
        ...
        {
            _viewportZoomCache = CreateUndoRedoStackItem();
            if (_redoStack.Count == 0 || !_viewportZoomCache.Equals(_redoStack.Peek()))
            {
                _undoStack.Push(_viewportZoomCache);
            }
